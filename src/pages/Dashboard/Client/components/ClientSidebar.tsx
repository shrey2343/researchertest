import { LayoutDashboard, FolderKanban, CheckSquare, CreditCard, UserCircle2, Plus, LogOut, Menu, X, MessageCircle } from 'lucide-react';
...
export default function ClientSidebar({
  ...
  const menuItems = [
    ...
    { id: 'spending', icon: CreditCard, label: 'Billing Details' },
    ...
    <>
      ...
      >
        ...
        <nav className="space-y-1.5 flex-1 relative z-10">
          ...
          <button
            onClick={() => {
              navigate('/postprojectpage');
              if (onMobileMenuToggle) onMobileMenuToggle();
            }}
            className="w-full flex items-center justify-center gap-2 px-2.5 py-2 lg:px-4 lg:py-3 rounded-xl transition-all text-xs lg:text-base bg-gradient-to-r from-blue-600 to-blue-700 text-white hover:from-blue-700 hover:to-blue-800 shadow-md hover:shadow-lg hover:scale-105 duration-200 mt-3"
          >
            <Plus size={16} className="lg:w-5 lg:h-5 flex-shrink-0" />
            <span className="leading-none">Post Project</span>
          </button>
        </nav>
      </aside>
    </>
